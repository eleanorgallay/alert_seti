...
def DStars(dlim=1, verbose=False):
    '''
    function for Query for Disappearing Stars

    dlim: the number of days back to filter
    verbose: should we print stuff and plot stuff to the screen?
    '''

    mjdnow = str(int(Time.now().mjd)) # just the decimal MJD, used in file names for today
    jdnow = str(Time.now().jd)
    days = str(dlim) # number of days. THIS is the window of time to search Lasair
    ...
    L = lasair(API_TOKEN, endpoint = "https://lasair-ztf.lsst.ac.uk/api")
    ...
    gaiapath = '/Users/james/Dropbox/research_projects/lonely_stars/'
    #reading Gaia csv file
    # df = pd.read_csv(gaiapath + '/gaia.tsv',comment="#",delimiter=";")
    tbl = Table.read(gaiapath + 'GCNS_cat.fits',format='fits')
    gaia = tbl.to_pandas()
    ...
    BP = pd.to_numeric(gaia['PHOT_BP_MEAN_MAG'], errors='coerce') #BP magnitude
    RP = pd.to_numeric(gaia['PHOT_RP_MEAN_MAG'], errors='coerce') #RP magnitude
    color = BP - RP #BP-RP color

    gaia_gmag = pd.to_numeric(gaia['PHOT_G_MEAN_MAG'], errors='coerce') #apparent g magnitudes
    plx = pd.to_numeric(gaia['PARALLAX'], errors= 'coerce') #parallax in mas
    ...
    for obj in Dips:
        ...
        # NOTE: you need to log in once to IRSA, then it will remember
    ...
    plt.plot(trash_color, trash_gmag, 's', color = 'purple',  zorder= 10, markersize= 4, alpha= 0.8, label = 'Stars that passed Lasair filter')
    plt.plot(cand_color, cand_gmag, '*', color = 'red', zorder= 11, markersize = 11, alpha= 0.7, label = 'Stars that passed filter + statistical tests')
...
if __name__ == "__main__":
    import sys
    # can tell it to pass sys.argv values here to change command line run if we want
    DStars()
